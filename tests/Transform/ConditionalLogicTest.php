...
use Tsp\AcfBuilder\FlexibleContentBuilder;
...
class ConditionalLogicTest extends \PHPUnit_Framework_TestCase
{
    public function testIsIterative()
    {
        ...
        $transform = new Transform\ConditionalLogic($builder);

        $this->assertInstanceOf(Transform\IterativeTransform::class, $transform);
    }
    ...
    public function testTransformValue()
    {
        ...
        $field->method('getKey')->willReturn('field_name');
        $field->method('hasCustomKey')->willReturn(false);

        $builder = $this->createMock(FieldsBuilder::class);
        $builder->method('getField')->with('name')->willReturn($field);
        $builder->method('fieldExists')->with('name')->willReturn(true);
    ...
    {
        $builder = new FlexibleContentBuilder('name');
        $builder
            ->addLayout('hero')
                ->addImage( 'hero_image' )
